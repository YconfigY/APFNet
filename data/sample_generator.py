...
class SampleGenerator():
    def __init__(self, type_, img_size, trans=1, scale=1, aspect=None, valid=False):
        self.type = type_
        self.img_size = np.array(img_size)  # size of original image, (w, h)
        self.trans = trans
        self.scale = scale
        self.aspect = aspect
        self.valid = valid
    ...
    def _gen_samples(self, bbox, n):
        ...
        if self.type == 'gaussian':
            samples[:, :2] += self.trans * np.mean(bbox[2:]) * np.clip(
            ...
            m = int(2 * np.sqrt(n))
            xy = np.dstack(np.meshgrid(np.linspace(0, 1, m), 
    ...
    def __call__(self, bbox, n, overlap_range=None, scale_range=None):
        """Get positive or negative examples' bounding box.

        Args:
            bbox (list): grounding truth bounding box.
            n (int): the number of 'bbox'.
            overlap_range (list[float], optional): 
                the range of IoUs between '_samples' and 'bbox'. Defaults to None.
            scale_range (list[float], optional): 
                the range of (areas of '_samples' / areas of 'bbox'). Defaults to None.
