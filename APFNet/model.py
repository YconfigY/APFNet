...
class MDNet(nn.Module):
    def __init__(self, num_branches=1):
        ...
        self.layers_v, self.layers_i = [nn.Sequential(OrderedDict([
            ('conv1', nn.Sequential(
                ...
                nn.LocalResponseNorm(2), nn.MaxPool2d(kernel_size=3, stride=2))),
            ('conv2', nn.Sequential(
                nn.Conv2d(96, 256, kernel_size=5, stride=2), nn.ReLU(inplace=True),
                nn.LocalResponseNorm(2), nn.MaxPool2d(kernel_size=3, stride=2))),
            ('conv3', nn.Sequential(
                nn.Conv2d(256, 512, kernel_size=3, stride=1), nn.ReLU()))])) for _ in range(2)]
        ...
        if self.stage == 1:
            self.parallel1 = nn.Sequential(OrderedDict([
                ...
                # 5*5 kernel, 107*107 -> 52*52,(107-5)/2+1=52
                ('parallel1_conv2', nn.Sequential(nn.Conv2d(32, 96, kernel_size=4, stride=2)))]))
                # 4*4 kernel, 52*52 -> 25*25,(52-4)/2+1=25
            self.parallel2 = nn.Sequential(OrderedDict([
                ('parallel2_conv1', nn.Sequential(nn.Conv2d(96, 256, kernel_size=3, stride=2),  # 25->12,(25-3)/2+1=12
                                                  nn.MaxPool2d(kernel_size=8, stride=1)))]))  # 12->5,(12-8)/1+1=->5
            self.parallel3 = nn.Sequential(OrderedDict([
                ('parallel3_conv1', nn.Sequential(nn.Conv2d(256, 512, kernel_size=1, stride=1),  #5,(5-1)/1+1=5
            ...
            [nn.Sequential(OrderedDict([
                ...
                (f'parallel{_layer_indx+1}_skconv_fc2', nn.Sequential(
                     nn.Conv2d(_channel_a, _channel_b * 2, 1, 1, bias=False)))]))
        ...
        # fc6
        self.branches = nn.ModuleList([nn.Sequential(nn.Dropout(0.5), nn.Linear(512, 2)) for _ in range(num_branches)])

        self.blockes = {'parallel':{}, 'p_skconv':{}}
        for i, (parallel, p_skconv) in enumerate(zip(['parallel1','parallel2','parallel3'], 
                                                     ['parallel1_skconv', 'parallel2_skconv', 'parallel3_skconv'])):
    ...
    def get_dict_stage(self):
        dict_stage = {'conv1':{}, 'conv2':{}, 'conv3':{}}
        dict_stage = EasyDict(dict_stage)
        for i, channel in enumerate([96, 256, 512]):
    ...
    def forward(self, img_v, img_i, k=0, in_layer='conv1', out_layer='fc6'):
        ...
        for (name_v, module_v), (name_i, module_i) in zip(
            ...
            if run:
                ...
                if name_v == 'conv3' or self.stage >= 3:
